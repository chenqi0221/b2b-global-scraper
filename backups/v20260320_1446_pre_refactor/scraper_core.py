        ...
        if mode == "append" and files:
            try:
                ...
                if existing_values and len(existing_values) > 1:
                    ...
                    if subset:
                        final_df = combined_df.drop_duplicates(subset=subset, keep='first')
                    else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if len(data) > chunk_size:
                    update_gui_callback(f"数据量较大 ({len(data)}行)，采用分块上传模式...")
                ...
                if "500" in str(update_err) and attempt < max_retries - 1:
                    ...
                    update_gui_callback(f"云端繁忙 (500)，{wait_time}秒后进行第 {attempt + 2} 次重试...")
    ...
    try:
        ...
        if results:
            ...
            try:
                ...
                # 使用 reindex 解决 KeyError，如果某个字段全为空也会正确生成空列
