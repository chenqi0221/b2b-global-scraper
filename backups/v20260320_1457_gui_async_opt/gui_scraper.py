...
class ScraperGUI:
    ...
    def _setup_ui(self):
        ...
        # AI 拓展关键词功能区
        ai_frame = ttk.LabelFrame(main_frame, text="AI 拓展关键词 (可选)", padding="5")
        ai_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(ai_frame, text="输入产品/种子词:").pack(side=tk.LEFT, padx=2)
        self.seed_kw_entry = ttk.Entry(ai_frame, width=20)
        self.seed_kw_entry.pack(side=tk.LEFT, padx=2)
        
        ttk.Label(ai_frame, text="生成数量:").pack(side=tk.LEFT, padx=2)
        self.ai_num_cb = ttk.Combobox(ai_frame, values=[str(i) for i in range(1, 21)], width=5, state="readonly")
        self.ai_num_cb.set("7")
        self.ai_num_cb.pack(side=tk.LEFT, padx=2)
        
        self.ai_gen_btn = ttk.Button(ai_frame, text="AI 生成关键词", command=self.generate_ai_keywords)
        ...
        ttk.Label(row1, text="大洲:").pack(side=tk.LEFT)
        self.continent_cb = ttk.Combobox(row1, values=list(GEOGRAPHICAL_DATA.keys()), state="readonly", width=15)
        self.continent_cb.pack(side=tk.LEFT, padx=(5, 15))
        self.continent_cb.bind("<<ComboboxSelected>>", self._on_continent_select)

        ttk.Label(row1, text="国家:").pack(side=tk.LEFT)
        self.country_cb = ttk.Combobox(row1, state="readonly", width=15)
        ...
        ttk.Label(row2, text="城市:").pack(side=tk.LEFT)
        self.city_cb = ttk.Combobox(row2, state="readonly", width=15)
        self.city_cb.pack(side=tk.LEFT, padx=(5, 15))
        self.city_cb.bind("<<ComboboxSelected>>", self._on_city_select)

        ttk.Label(row2, text="区域:").pack(side=tk.LEFT)
        self.district_cb = ttk.Combobox(row2, state="readonly", width=15)
    ...
    def generate_ai_keywords(self):
        """AI 拓展关键词的回调函数"""
        seed_word = self.seed_kw_entry.get().strip()
        num = self.ai_num_cb.get()
        
        if not seed_word:
            messagebox.showwarning("提示", "请输入产品/种子词")
            return
            
        self.ai_gen_btn.config(state=tk.DISABLED)
    ...
    def _ai_worker(self, seed_word, num):
        """后台 AI 生成任务"""
        try:
            # 这里将在下一步编写具体的 API 调用逻辑
            # 目前先留出占位符，或直接调用核心层的相关函数
            from scraper_core import generate_keywords_with_ai
            
            # 运行同步/异步 API 调用
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            keywords = loop.run_until_complete(generate_keywords_with_ai(seed_word, int(num)))
            loop.close()
            
            if keywords:
                def _update_ui():
                    # 将生成的关键词填充到 kw_text 区域
                    self.kw_text.delete("1.0", tk.END)
                    self.kw_text.insert(tk.END, "\n".join(keywords))
                    self.update_status(f"AI 成功生成 {len(keywords)} 个关键词")
                self.root.after(0, _update_ui)
            else:
                self.update_status("AI 未能生成关键词，请检查 API 配置或代理")
                
        except Exception as e:
            self.update_status(f"AI 生成失败: {str(e)}")
        finally:
            self.root.after(0, lambda: self.ai_gen_btn.config(state=tk.NORMAL))

    def _on_continent_select(self, event):
        continent_key = self.continent_cb.get()
    ...
    def start_task(self):
        # 校验输入
        if not self.continent_cb.get() or not self.country_cb.get() or not self.city_cb.get():
            messagebox.showwarning("提示", "请选择完整的大洲、国家和城市信息")
            return
    ...
    async def main_worker(self):
        ...
        if not all([continent_key, country_key, city_key]):
            messagebox.showwarning("提示", "请选择完整的大洲、国家和城市信息")
            self._reset_ui()
            return
        ...
        async with async_playwright() as p:
            ...
            # 所有关键词抓取完成后，自动执行汇总并同步至云端
            if self.is_running:
                try:
                    self.update_status("正在执行自动汇总与同步...")
                    await aggregate_and_sync(output_dir, self.update_status, target_title="lengdangb2b")
                    self.update_status("自动汇总与同步已完成")
                except Exception as ae:
                    self.log_message(f"自动汇总同步失败: {str(ae)}")
